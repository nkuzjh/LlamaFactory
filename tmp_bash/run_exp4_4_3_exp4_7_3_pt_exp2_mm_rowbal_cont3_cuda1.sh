...
# PT-exp2-mm-rowbal-cont3 ep3 downstream is a fixed physical-CUDA-1 serial
...
LOCK_FILE="${SCRIPT_DIR}/run_exp4_4_3_exp4_7_3_pt_exp2_mm_rowbal_cont3_cuda1.lock"
LOG_FILE="${SCRIPT_DIR}/run_exp4_4_3_exp4_7_3_pt_exp2_mm_rowbal_cont3_cuda1.log"
PID_FILE="${SCRIPT_DIR}/run_exp4_4_3_exp4_7_3_pt_exp2_mm_rowbal_cont3_cuda1.pid"
...
on_exit() {
    ...
    log "PT-exp2-mm-rowbal-cont3 downstream batch exit status=${exit_code}"
}

trap 'on_error "$?" "$LINENO"' ERR
trap on_exit EXIT

log "PT-exp2-mm-rowbal-cont3 downstream batch start preflight_only=${PREFLIGHT_ONLY} pid=$$"
...
run_stage() {
    ...
    local result_file
    result_file="$(mktemp "${TMPDIR:-/tmp}/exp4_4_3_exp4_7_3_preflight.XXXXXX")"
...
if payload.get("action") != expected_action or payload.get("gpus") != "1":
    raise SystemExit("preflight action/GPU contract mismatch")
...
log "PT-exp2-mm-rowbal-cont3 downstream batch complete"
